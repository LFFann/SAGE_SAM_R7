...
def test_r7_v100_config_uses_adapter_only_verifier_and_trust_gate():
    ...
    assert cfg["losses"]["class_balanced_ce"]["max_foreground_weight"] <= 1.35
    assert cfg["losses"]["ssl_class_balance"]["enabled"] is True
    assert 0.0 < cfg["losses"]["ssl_class_balance"]["foreground_power"] <= 0.75
    ...
    assert cfg["losses"]["hard_pseudo_reliability"]["enabled"] is True
    assert cfg["losses"]["hard_pseudo_reliability"]["start_iter"] >= cfg["r6"]["foreground_grounding_start"]
    assert 0.0 < cfg["losses"]["hard_pseudo_reliability"]["background_scale"] <= 0.70
    assert cfg["losses"]["hard_pseudo_reliability"]["background_min_scale"] <= cfg["losses"]["hard_pseudo_reliability"]["background_scale"]
    assert 0.0 < cfg["losses"]["hard_pseudo_reliability"]["foreground_min_weight"] <= 0.35
    assert cfg["losses"]["hard_pseudo_reliability"]["demote_low_foreground"] is True
    ...
    assert 0.0 < cfg["losses"]["strong_view_consistency"]["weight"] <= 0.05
...
def test_r7_launch_scripts_are_parameterized():
    ...
    assert "losses.hard_pseudo_reliability.enabled false" in ablate_script
