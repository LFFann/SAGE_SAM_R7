...
ABLATIONS="${ABLATIONS:-full no_sam no_prior_feedback no_copy_paste no_strong_consistency no_trust_conditioned_floor no_ssl_class_balance no_hard_pseudo_reliability no_prototype_anchor no_topology_filter no_prompt_consistency no_eval_topology no_boundary no_class_balance}"
...
for name in ${ABLATIONS}; do
  ...
  case "${name}" in
    ...
    no_ssl_class_balance)
      ...
      ;;
    no_hard_pseudo_reliability)
      python tools/verify_real_sam.py --config "${CONFIG}"
      args+=(--opts losses.hard_pseudo_reliability.enabled false)
      ;;
    no_prototype_anchor)
      python tools/verify_real_sam.py --config "${CONFIG}"
      args+=(--opts losses.foreground_prototype.enabled false)
      ;;
    no_topology_filter)
      python tools/verify_real_sam.py --config "${CONFIG}"
